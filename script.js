...
let POP_SOUND = new Audio('pop2.mp3');
POP_SOUND.volume = 0.05;

let AUDIO_CONTEXT = new (AudioContext || webkitAudioConext || window.webkitAudioConext)();
let keys = {
  DO: 261.6,
  RE: 520,
  MI: 329.6,
...
function formatTime(milsec) {
  let seconds = Math.floor(milsec / 1000);
...
function onMouseDown(evt) {
  ...
  SELECTED_PIECE = getPressedPieceByColour(evt, clickedColour);
...
function getPressedPieceByColour(loc, col) {
  for (let i = PIECES.length - 1; i >= 0; i--) {
    if (PIECES[i].colour == col) {
      return PIECES[i];
    }
  }
  return null;
}
// Resize to garantee ratio
...
// Draw video on Canvas with updateFx, called on every frame
function updateGame() {
...
function initializePieces(rows, cols) {
  ...
  SIZE.columns = cols;
  PIECES = [];
  const uniqueRandomColours = [];
  for (let i = 0; i < SIZE.rows; i++) {
    for (let j = 0; j < SIZE.columns; j++) {
      let colour = getRandomColour();
      //  uniqueRandomColours.push(colour);
      while (uniqueRandomColours.includes(colour)) {
...
function playMelodie() {
  playNote(keys.DO, 200);
  setTimeout(() => {
    playNote(keys.MI, 100);
  }, 250);
  setTimeout(() => {
